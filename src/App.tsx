...
async function parse3MF(buf: ArrayBuffer): Promise<Mesh> {
  ...
  if (!JSZip) {
    throw new Error("JSZip is not loaded. Please wait or check your internet connection.");
  }
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (gl && prog && triBuf) {
        ...
        gl.uniform1f(gl.getUniformLocation(prog, "uRotX"), rotXRef.current);
        gl.uniform1f(gl.getUniformLocation(prog, "uRotY"), rotYRef.current);
        gl.uniform1f(gl.getUniformLocation(prog, "uZoom"), zoomRef.current);
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
      <div className="card">
        ...
        <p className="subtitle">
          Load an STL or 3MF file, preview in 3D, set your material thickness, and download per-slice SVGs.
        </p>
        ...
        <div className={`progress-bar ${progress !== null ? 'active' : ''}`}>
